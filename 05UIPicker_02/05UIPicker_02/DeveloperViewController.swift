...
class DeveloperViewController: UIViewController {
    ...
    let technologyField: UITextField = {
        let tf = UITextField(placeHolder: "Технология")
        tf.textAlignment = .left
        tf.translatesAutoresizingMaskIntoConstraints = false
        return tf
    }()
    
    let experiencelextField: UITextField = {
        let tf = UITextField(placeHolder: "Опыт в годах")
        tf.textAlignment = .left
        tf.translatesAutoresizingMaskIntoConstraints = false
        return tf
    }()
    ...
    override func viewDidLoad() {
        ...
        self.title = "Регистрация разработчика"
        ...
        view.addSubview(technologyField)
        view.addSubview(experiencelextField)
    ...
    private func setupLayout() {
        ...
        technologyField.topAnchor.constraint(equalTo: emailextField.bottomAnchor, constant: 30).isActive = true
        technologyField.leftAnchor.constraint(equalTo: firstNameTextField.leftAnchor).isActive = true
        technologyField.rightAnchor.constraint(equalTo: firstNameTextField.rightAnchor).isActive = true
        technologyField.heightAnchor.constraint(equalTo: firstNameTextField.heightAnchor).isActive = true
        
        experiencelextField.topAnchor.constraint(equalTo: technologyField.bottomAnchor, constant: 30).isActive = true
        experiencelextField.leftAnchor.constraint(equalTo: firstNameTextField.leftAnchor).isActive = true
        experiencelextField.rightAnchor.constraint(equalTo: firstNameTextField.rightAnchor).isActive = true
        experiencelextField.heightAnchor.constraint(equalTo: firstNameTextField.heightAnchor).isActive = true
        
        saveButton.topAnchor.constraint(equalTo: experiencelextField.bottomAnchor, constant: 40).isActive = true
    ...
    @objc private func handleSave(_sender: AnyObject) {
        ...
        if firstNameTextField.text != "" && secondNameTextField.text != "" && passwordTextField.text != "" && bankCardTextField.text != "" && emailextField.text != "" && technologyField.text != "" && experiencelextField.text != "" {
            stopButton(self, title: "Ok", message: "Данные сохранены.")
        } else {
