//
//  ThreeTariffViewController.swift
//  04_UISwitch
//
...
class ThreeTariffViewController: UIViewController {
    ...
    func setup() {
        ...
        threeZoneLabel.textAlignment = .center
        threeZoneLabel.text = "Зона Т3 (10.00-17.00; 21.00-23.00)"
        
        threeZoneTextField.placeholder = "Введите значение..."
        threeZoneTextField.borderStyle = UITextBorderStyle.roundedRect
        threeZoneTextField.autocorrectionType = UITextAutocorrectionType.no
        threeZoneTextField.keyboardType = UIKeyboardType.numberPad
        threeZoneTextField.returnKeyType = UIReturnKeyType.done
        threeZoneTextField.clearButtonMode = UITextFieldViewMode.whileEditing
        threeZoneTextField.widthAnchor.constraint(equalToConstant: 20)
        threeZoneTextField.heightAnchor.constraint(equalToConstant: 50)
        threeZoneTextField.delegate = self
        
        let stackViewGroup = UIStackView(arrangedSubviews: [oneZoneLabel, oneZoneTextField, twoZoneLabel, twoZoneTextField, threeZoneLabel, threeZoneTextField, resultLabel])
    ...
    @objc func getResult() -> Double {
        ...
        if (oneZoneTextField.text?.isEmpty) ?? true || (twoZoneTextField.text?.isEmpty) ?? true || (threeZoneTextField.text?.isEmpty) ?? true{
            let alert = UIAlertController(title: "Значения Т1, T2, T3 не могут быть пустыми", message: nil, preferredStyle: .alert)
            alert.addAction(UIAlertAction(title: "Ok", style: .cancel, handler: nil))
            self.present(alert, animated: true)
        }
        guard let valueT1 = Double(self.oneZoneTextField.text!), let valueT2 = Double(self.twoZoneTextField.text!), let valueT3 = Double(self.twoZoneTextField.text!) else { return 0 }
        return (costT1 * valueT1) + (costT2 * valueT2) * (costT1 * valueT3)
    }
}
